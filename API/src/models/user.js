...
const userModel = {  async findByUsername(username) {
    const query = 'SELECT * FROM "postgres"."User" WHERE "User_Username" = $1';
    const { rows } = await db.query(query, [username]);
    return rows[0];
  },
  
  async findByEmail(email) {
    const query = 'SELECT * FROM "postgres"."User" WHERE "User_Email" = $1';
  ...
  async findById(email) {
    ...
    const query = 'SELECT "User_Preferences" FROM "postgres"."User" WHERE "User_Email" = $1';
  ...
  async validateAPIKey(apiKey) {
    const query = 'SELECT * FROM "postgres"."User" WHERE "User_APIKey" = $1';
