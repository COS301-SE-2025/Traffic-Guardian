...
class EnhancedCrashClassifier:
    ...
    CRASH_TYPES = {
        ...
        4: "single_vehicle_rollover",  # Single vehicle rollover
        5: "vehicle_pedestrian",       # Vehicle vs pedestrian
        6: "vehicle_fixed_object",     # Vehicle vs fixed object
        7: "sideswipe_collision",      # Sideswipe collision
        8: "intersection_collision",   # Intersection collision
        9: "highway_collision",        # Highway/high-speed collision
        10: "parking_lot_incident"     # Low-speed parking lot incident
    }
    ...
    def extract_enhanced_sequence(self, video_path: str) -> Tuple[torch.Tensor, Dict]:
        ...
        if total_frames > 0:
            # Ensure we capture the key moments of a crash
            step = max(1, (total_frames - sequence_length * frame_skip) // sequence_length)
            step = max(step, frame_skip)
        else:
            step = frame_skip
        ...
        try:
            while len(frames) < sequence_length:
                ...
                if frame_count % step == 0:
                    # Enhanced preprocessing based on video quality
                    enhanced_frame = self.preprocessor.enhance_frame_quality(
                        frame, enhancement_level
                    )
    ...
    def analyze_crash_motion(self, video_path: str) -> Dict:
        ...
        try:
            while True:
                ...
                video_quality = self.preprocessor.detect_video_quality(video_path)
                enhancement_level = 'heavy' if video_quality == 'poor' else 'medium'
    ...
    def _detect_vehicles_enhanced(self, frame: np.ndarray, fg_mask: np.ndarray) -> List[Dict]:
        ...
        try:
            ...
            # Sort contours by area (largest first) and limit to reasonable number
            contours = sorted(contours, key=cv2.contourArea, reverse=True)
            max_vehicles_to_detect = 15  # Reasonable upper limit for vehicles in frame
            
            for i, contour in enumerate(contours[:max_vehicles_to_detect]):
                ...
                if area > 1000:  # Minimum vehicle area
                    ...
                    if 0.5 < aspect_ratio < 3.0 and w > 40 and h > 30:
                        # Avoid duplicate detections by checking overlap with existing vehicles
                        is_duplicate = False
                        for v in vehicles:
                            # Calculate IoU with existing vehicles
                            x1, y1, w1, h1 = v['bbox']
                            intersection_area = max(0, min(x+w, x1+w1) - max(x, x1)) * \
                                                max(0, min(y+h, y1+h1) - max(y, y1))
                            if intersection_area > 0.5 * min(w*h, w1*h1):
                                is_duplicate = True
                                break
            ...
            # Only use template matching if we haven't found enough vehicles
            if len(vehicles) < 2:
                gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                
                # Create simple vehicle templates
                vehicle_templates = self._create_vehicle_templates()
                
                for template_name, template in vehicle_templates.items():
                    if template is not None:
                        result = cv2.matchTemplate(gray, template, cv2.TM_CCOEFF_NORMED)
                        threshold = 0.65  # Higher threshold for better precision
                        locations = np.where(result >= threshold)
                        
                        # Limit template detections
                        for pt_idx, pt in enumerate(list(zip(*locations[::-1]))[:5]):
                            h, w = template.shape
                            ...
                            if not is_duplicate:
                                vehicles.append({
                                    'id': f'template_{template_name}_{pt_idx}',
                                    'bbox': (pt[0], pt[1], w, h),
    ...
    def _classify_crash_type_enhanced(self, motion_data: List[Dict], 
        ...
        # Enhanced classification logic specifically for T-bone detection
        if is_tbone_pattern and 1 <= max_vehicles <= 4:
            # This is likely a T-bone collision
            return "tbone_side_impact"
            
        # Classification based on vehicle count and impact characteristics
        if max_vehicles <= 1:
            ...
            if max_impact_severity in ['critical', 'high']:
                # T-bone detection logic (perpendicular impact)
                if is_tbone_pattern or motion_variance > 50000000:
                    return "tbone_side_impact"
                ...
                if is_tbone_pattern:
                    return "tbone_side_impact"  # Still T-bone but lower severity
                return "sideswipe_collision"
                
        else:  # 3+ vehicles
            if max_impact_severity == 'critical' and max_vehicles > 4:
                return "multi_vehicle_pileup"
            else:
                # With 3-4 vehicles, check for T-bone with secondary vehicles
                if is_tbone_pattern:
                    return "tbone_side_impact"
    ...
    def classify_crash_video(self, video_path: str, camera_id: str = None) -> CrashReport:
        """
        ...
        try:
            ...
            with torch.no_grad():
                ...
                # Reshape the 5D tensor to 4D by merging frames and batch dimensions
                # Then later we'll process one frame at a time
                frame_predictions = []
            ...
            logger.error(f"Error analyzing crash video {video_path}: {e}")
            raise
    ...
    def _generate_detailed_alert_message(self, crash_type: str, severity: str, 
        ...
        crash_descriptions = {
            "tbone_side_impact": f"T-bone/side-impact collision involving {vehicles_involved} vehicle(s)",
            "rear_end_collision": f"Rear-end collision involving {vehicles_involved} vehicle(s)",
            "head_on_collision": f"Head-on collision between {vehicles_involved} vehicle(s)",
            ...
            "sideswipe_collision": f"Sideswipe collision involving {vehicles_involved} vehicle(s)",
            "intersection_collision": f"Intersection collision involving {vehicles_involved} vehicle(s)",
            "highway_collision": f"Highway collision involving {vehicles_involved} vehicle(s)",
            "parking_lot_incident": f"Low-speed parking lot incident involving {vehicles_involved} vehicle(s)"
        }
        
        base_message = crash_descriptions.get(crash_type, f"Traffic collision involving {vehicles_involved} vehicle(s)")
        ...
        emergency_recommendations = {
            'critical': "DISPATCH: Fire, EMS (multiple units), Police, Traffic Control",
    ...
    def _estimate_crash_speed(self, motion_analysis: Dict) -> str:
        ...
        if max_motion > 50000:
            return "high_speed"  # >45 mph
        elif max_motion > 25000:
            return "medium_speed"  # 25-45 mph
        elif max_motion > 10000:
            return "low_speed"  # 15-25 mph
        else:
            return "very_low_speed"  # <15 mph
    ...
    def process_crash_folder(self, folder_path: str, camera_id: str = None) -> Dict:
        """
        ...
        for video_file in video_files:
            try:
                ...
                crash_report = self.classify_crash_video(video_file, camera_id)
