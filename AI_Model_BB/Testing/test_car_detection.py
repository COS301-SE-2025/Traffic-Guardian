...
try:
    ...
    print(f"Files in Code folder: {os.listdir(CODE_FOLDER) if os.path.exists(CODE_FOLDER) else 'Folder not found'}")
    sys.exit(1)
...
class TestVideoPathHandling(unittest.TestCase):
    ...
    def test_video_file_access(self):
        ...
        if os.path.exists(videos_path):
            video_files = [f for f in os.listdir(videos_path) if f.endswith(('.mp4', '.avi', '.mov'))]
