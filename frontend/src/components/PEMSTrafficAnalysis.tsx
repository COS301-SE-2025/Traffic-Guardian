...
const PEMSTrafficAnalysis: React.FC<Props> = ({ 
  ...
  const [selectedTab, setSelectedTab] = useState<'overview' | 'alerts' | 'detectors' | 'recommendations'>('overview');
    ...
    <div className="pems-analysis">
      <div className="pems-header">
        ...
        <div className="pems-status">
          <div className={`system-status ${getSystemHealthClass(pemsData.summary.system_health)}`}>
      ...
      <div className="pems-metrics">
        <div className="metric-card">
          ...
          <div className="metric-content">
            <div className="metric-value">{pemsData.summary.total_detectors}</div>
            <div className="metric-label">Active Detectors</div>
            <div className="metric-subtitle">{pemsData.summary.active_detectors} operational</div>
          </div>
        </div>
        ...
        <div className="metric-card">
          ...
          <div className="metric-content">
            <div className="metric-value">{pemsData.summary.avg_speed.toFixed(1)} mph</div>
        ...
        <div className="metric-card">
          ...
          <div className="metric-content">
            <div className="metric-value">{pemsData.summary.total_flow.toLocaleString()}</div>
            <div className="metric-label">Vehicle Flow</div>
            <div className="metric-subtitle">Vehicles per hour</div>
          </div>
        </div>

        <div className={`metric-card ${getRiskLevelClass(pemsData.summary.avg_risk_score > 7 ? 'HIGH' : pemsData.summary.avg_risk_score > 5 ? 'MEDIUM' : 'LOW')}`}>
          <div className="metric-icon">
            <AlertTriangleIcon />
          </div>
          <div className="metric-content">
            <div className="metric-value">{pemsData.summary.high_risk_count}</div>
            <div className="metric-label">High Risk Areas</div>
            <div className="metric-subtitle">Risk Score: {pemsData.summary.avg_risk_score.toFixed(1)}/10</div>
          </div>
        </div>
      </div>
              ...
              {pemsData.critical_areas.length > 0 ? (
                <div className="critical-areas-list">
                  {pemsData.critical_areas.map((area, index) => (
                    <div key={index} className={`critical-area-card ${getRiskLevelClass(area.risk_level)}`}>
            ...
            <div className="risk-distribution">
              ...
              <div className="risk-bars">
                {['CRITICAL', 'HIGH', 'MEDIUM', 'LOW'].map(level => {
                  const count = pemsData.detectors.filter(d => d.risk_level === level).length;
                  ...
                  return (
                    <div key={level} className="risk-bar">
                      ...
                      <div className="risk-percentage">{percentage.toFixed(1)}%</div>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="alerts-section">
            <div className="alerts-header">
              ...
              <div className="alert-summary">
                <span className="priority-count high">High: {pemsData.alerts.filter(a => a.priority === 'HIGH').length}</span>
                <span className="priority-count medium">Medium: {pemsData.alerts.filter(a => a.priority === 'MEDIUM').length}</span>
                <span className="priority-count low">Low: {pemsData.alerts.filter(a => a.priority === 'LOW').length}</span>
              </div>
            </div>
                    ...
                    >
                      <div className="alert-header">
                        <div className="alert-info">
                          <span className="detector-id">{alert.detector_id}</span>
                          <span className="freeway-direction">{alert.freeway} {alert.direction}</span>
                        </div>
                        <div className="alert-meta">
                          <span className="priority-badge">{alert.priority}</span>
                          <span className="risk-score">Risk: {alert.risk_score.toFixed(1)}</span>
                        </div>
                      </div>
              ...
              {pemsData.high_risk_areas.length > 0 ? (
                pemsData.high_risk_areas.map((detector) => (
                  <div 
                    ...
                    className={`detector-card ${getRiskLevelClass(detector.risk_level)}`}
                    onClick={() => onDetectorSelect?.(detector)}
                  >
                    <div className="detector-header">
                      <span className="detector-id">{detector.detector_id}</span>
                      <span className="risk-level-badge">{detector.risk_level}</span>
                    </div>
                    <div className="detector-location">
                      {detector.freeway} {detector.direction} • {detector.detector_type}
                    </div>
                    <div className="detector-metrics">
                      <div className="metric-row">
                        <span>Speed:</span>
                        <span className={detector.speed < 20 ? 'metric-warning' : ''}>
                      ...
                      <div className="metric-row">
                        <span>Occupancy:</span>
                        <span className={detector.occupancy > 40 ? 'metric-warning' : ''}>
                          {detector.occupancy.toFixed(1)}%
                        </span>
                      </div>
                      <div className="metric-row">
                        <span>Risk Score:</span>
                        <span className="risk-score-value">{detector.risk_score.toFixed(1)}/10</span>
                      </div>
                    </div>
                  </div>
                ))
      ...
      <div className="pems-footer">
        ...
        <span className="data-source">Data Source: Caltrans PEMS District {pemsData.district}</span>
      </div>
    </div>
  );
};
