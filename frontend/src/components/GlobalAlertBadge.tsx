...
const GlobalAlertBadge: React.FC = () => {
  ...
  if (location.pathname === '/' || location.pathname === '/account' || location.pathname === '/signup') {
    return null;
  }
  ...
  return (
    <>
      ...
      <div className="global-alert-badge">
        {/* Alerts Button */}
        <button 
          className={`global-alerts-btn ${unreadAlertCount > 0 ? 'has-alerts' : ''}`}
      ...
      {/* Alert Panel */}
      {showPanel && (
        <div className="global-alert-panel-overlay" onClick={() => setShowPanel(false)}>
          <div className="global-alert-panel" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="global-alert-panel-actions">
                {realtimeAlerts.length > 0 && (
                  <>
                    ...
                    <button className="btn-link-global" onClick={clearAllAlerts}>
                      Clear all
                    </button>
                  </>
                )}
                <button className="btn-close-global" onClick={() => setShowPanel(false)}>
                  <XIcon />
                </button>
              </div>
            </div>
            ...
            <div className="global-alert-panel-content">
              {realtimeAlerts.length === 0 ? (
                <div className="global-alerts-empty">
                  ...
                  <p className="empty-subtitle">New incidents will appear here in real-time</p>
                  <button 
                ...
                <>
                  <div className="global-alerts-list">
                    {realtimeAlerts.slice(0, 10).map((alert: RealTimeAlert) => (
                      <div 
                        key={alert.id} 
                        className={`global-alert-item ${alert.acknowledged ? 'acknowledged' : 'unread'} severity-${alert.incident.Incident_Severity}`}
                      >
                        <div className="global-alert-content">
                          <div className="global-alert-header">
                            <span className={`global-alert-severity ${alert.incident.Incident_Severity}`}>
                              {getSeverityIcon(alert.incident.Incident_Severity)} {getSeverityDisplay(alert.incident.Incident_Severity)}
                            </span>
