...
import ApiService, { DatabaseIncident, TrafficIncident } from '../services/apiService';
...
const Analytics: React.FC = () => {
  ...
  const loadAnalyticsData = async () => {
    ...
    try {
      ...
      console.log('Fetching data from both database and traffic API...');
      const [dbData, trafficData] = await Promise.all([
        ApiService.fetchIncidents(),
        ApiService.fetchTrafficIncidents()
      ]);

      console.log('Data fetched successfully:', {
        dbIncidents: dbData.length,
        trafficLocations: trafficData.length
      });
      ...
      // Calculate traffic totals and process categories
      let trafficTotal = 0;
      let trafficCritical = 0;
      const categoryMap = new Map<string, number>();
      
      trafficData.forEach(locationData => {
        locationData.incidents.forEach(incident => {
          trafficTotal++;
          
          // Count critical incidents (magnitudeOfDelay >= 3)
          if (incident.properties.magnitudeOfDelay >= 3) {
            trafficCritical++;
          }
      ...
      console.log('Traffic stats:', { total: trafficTotal, critical: trafficCritical, categories: categoryMap.size });
      ...
      const categories: CategoryBreakdown[] = Array.from(categoryMap.entries())
        .map(([category, count]) => ({
          category,
          count,
          percentage: trafficTotal > 0 ? Math.round((count / trafficTotal) * 100) : 0,
        }))
      ...
      const locations: LocationHotspot[] = trafficData
        .map(locationData => ({
          location: locationData.location,
          incidents: locationData.incidents.length,
          avgSeverity: locationData.incidents.length > 0 
            ? locationData.incidents.reduce((sum, incident) => 
                sum + incident.properties.magnitudeOfDelay, 0) / locationData.incidents.length
            : 0
        }))
      ...
      <div className={`analytics-container ${isDarkMode ? 'dark-mode' : 'light-mode'}`}>
        <div className="analytics-loading">
          ...
          <p style={{ fontSize: '0.875rem', opacity: 0.7, marginTop: '0.5rem' }}>
            Fetching data from database and traffic API...
          </p>
        </div>
      </div>
    );
