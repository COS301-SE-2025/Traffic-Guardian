...
class DataPrefetchService {
  ...
  private prefetchConfigs: PrefetchConfig[] = [
    ...
    { endpoint: '/api/incidents', cacheKey: 'incidents', refreshInterval: 2 * 60 * 1000, priority: 'high' },
    { endpoint: '/api/archives/stats', cacheKey: 'archiveStats', refreshInterval: 5 * 60 * 1000, priority: 'high' },
    { endpoint: '/api/pems/dashboard-summary', cacheKey: 'pemsDashboard', refreshInterval: 3 * 60 * 1000, priority: 'high' },
    { endpoint: '/api/pems/high-risk-areas', cacheKey: 'highRiskAreas', refreshInterval: 4 * 60 * 1000, priority: 'high' },

    // Traffic data - normal priority
    { endpoint: '/api/traffic/incidentLocations', cacheKey: 'incidentLocations', refreshInterval: 3 * 60 * 1000, priority: 'normal' },
    { endpoint: '/api/traffic/criticalIncidents', cacheKey: 'criticalIncidents', refreshInterval: 3 * 60 * 1000, priority: 'normal' },
    { endpoint: '/api/traffic/incidentCategory', cacheKey: 'incidentCategory', refreshInterval: 5 * 60 * 1000, priority: 'normal' },

    // PEMS district data - normal priority
    { endpoint: '/api/pems/district/4', cacheKey: 'pems-district-4', refreshInterval: 5 * 60 * 1000, priority: 'normal' },
    { endpoint: '/api/pems/district/7', cacheKey: 'pems-district-7', refreshInterval: 5 * 60 * 1000, priority: 'normal' },
    { endpoint: '/api/pems/district/11', cacheKey: 'pems-district-11', refreshInterval: 5 * 60 * 1000, priority: 'normal' },
    { endpoint: '/api/pems/district/12', cacheKey: 'pems-district-12', refreshInterval: 5 * 60 * 1000, priority: 'normal' },

    // Analytics data - lower priority
    { endpoint: '/api/archives?limit=1000', cacheKey: 'archiveData', refreshInterval: 10 * 60 * 1000, priority: 'low' },
    { endpoint: '/api/pems/alerts', cacheKey: 'pemsAlerts', refreshInterval: 3 * 60 * 1000, priority: 'low' },
  ];
  ...
  private async safeFetch(url: string, options: RequestInit = {}): Promise<any | null> {
    try {
  ...
  getCacheStatus() {
    ...
    this.prefetchConfigs.forEach(config => {
      ...
      status[config.cacheKey] = {
        ...
        ageSeconds: cached ? Math.floor((Date.now() - cached.lastFetched) / 1000) : null
      };
    });
  ...
  async preloadPageData(pageName: string) {
    const pageConfigs: Record<string, string[]> = {
      dashboard: ['incidents', 'archiveStats', 'pemsDashboard', 'highRiskAreas'],
